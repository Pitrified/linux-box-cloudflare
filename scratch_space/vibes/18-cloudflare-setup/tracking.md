...
| 4  | cloudflared + fresh tunnel + DNS   | [`05-phase4-tunnel.md`](05-phase4-tunnel.md) | done     |
| 5  | Zero Trust (email OTP) + HSTS      | [`06-phase5-zero-trust.md`](06-phase5-zero-trust.md) | done     |
...
- 2026-07-15 : Phase 4 executed. Pre-flight: HSTS confirmed off in the dashboard (matches probes). On `g7`: `cert.pem` (March) still authenticates; created tunnel `pmn-14g4` = `ea1edca9-e52d-4ac8-927d-e43fc7010af4`, JSON scp'd to the box (`/etc/cloudflared/`, root:600, password-manager backup taken). Box: cloudflared 2026.7.1 from the apt repo - **guide snag: Ubuntu 26.04 codename `resolute` is not on pkg.cloudflare.com (404), used `noble` in the sources line** (candidate guide delta). `config.yml` rewritten (new UUID, `entries.*` ingress dropped until a backend exists) and committed (`3f96a14`), deployed via the diff gate, service installed + enabled, 3 QUIC edge connections registered. Repoint via `route dns --overwrite-dns` on g7; end-to-end verified: Access challenge -> email OTP login -> landing page + fonts served through the tunnel (nginx access log). Decommission: old tunnel `68aa8138` deleted (creds revoked), `tunnel list` shows only `ea1edca9`, stale `ssh.*`/`entries.*` CNAMEs deleted in dashboard, old JSON backup rotated, old box disk wiped. **Phase 4 done.** Next: Phase 5 immediately (Access review + HSTS enable).
- 2026-07-15 : Phase 5 executed, same day as the repoint per the audit rule. The prior setup's Access apps survived and cover everything ("Home Portal" = apex, "Internal Apps" = wildcard, both policy "Allow Friends"); nothing rebuilt. Decision: login stays **email OTP** (Cloudflare-only auth); Google OAuth IdP noted in the sub-plan as a possible later addition. Negative test: non-allowlisted email gets the code prompt but no code - Cloudflare's anti-enumeration, deny is implicit. Pre-HSTS HTTPS verification: valid auto-renewing Universal cert (LE, apex + wildcard), `http://` 301s to `https://`, TLS 1.2+ OK. HSTS enabled: max-age 6 months, includeSubDomains, preload off, no-sniff on - header verified at the edge. Tunnel-health notification created. **Phase 5 done. Phases 1-5 all done; the migration is complete.** Remaining: Phase 7 (draft, needs a bot backend decision) and folding guide deltas 3, 8, 9, 11, 12 into `docs/01_box_setup.md`.
- 2026-07-15 : folded all five remaining guide-gap deltas into `docs/01_box_setup.md`: 3 (`--overwrite-dns` for re-used zones, Phase 4.5), 8 (HSTS re-use/migration note + suggested settings, Phase 2.4), 9 (BFM/WAF vs Telegram webhook caveat, Phase 2.4), 11 (750-home ACL for `www-data`, Phase 3.2), 12 (apt codename-404 fallback, Phase 4.1). Delta table rows marked done; no guide-gap rows remain open. Outstanding for the effort: Phase 7 (draft) only. Push of today's commits happens from `g7`.
